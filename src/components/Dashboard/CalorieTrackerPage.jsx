...
const CalorieTrackerPage = ({ formData }) => {
  ...
  const initialNutrition = calculateNutrition(formData);
  const finalTargetCals = dynamicTarget || initialNutrition.targetCals;
  const targetPro = initialNutrition.targetPro;
  const targetCarbs = initialNutrition.targetCarbs;
  const targetFat = initialNutrition.targetFat;
...
Be accurate. Use standard serving sizes if the user doesn't specify amounts. All values should be integers.`;
  ...
  return (
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 140px)' }}>
      ...
      {/* Today's Log */}
      <TodaysLog entries={entries} handleDeleteEntry={handleDeleteEntry} />
