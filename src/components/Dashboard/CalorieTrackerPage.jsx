...
const CalorieTrackerPage = ({ formData }) => {
  ...
  const handleDeleteEntry = async (indexToRemove) => {
    ...
    if (user) {
      try {
        ...
        // Add a system message confirming deletion
        setMessages(prev => [...prev, { role: 'ai', text: `🗑️ Deleted: ${entryToRemove.meal}` }]);
      } catch (err) {
        console.warn("Could not delete calorie entry:", err);
        setError("Could not delete calorie entry. Please try again.");
      ...
      <div className="section-card" style={{ borderTop: '4px solid var(--accent-green)', flexShrink: 0 }}>
        ...
        {entries.length > 0 && (
          <div style={{ marginTop: '16px', borderTop: '1px solid var(--border)', paddingTop: '16px' }}>
            <h4 style={{ color: 'var(--text-primary)', marginBottom: '8px', fontSize: '0.9rem' }}>🍽️ Today's Log</h4>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '150px', overflowY: 'auto' }}>
              {entries.map((entry, idx) => (
                <div key={idx} style={{
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: '500', fontSize: '0.9rem', color: 'var(--text-primary)' }}>{entry.meal}</div>
                    <div style={{ fontSize: '0.75rem', color: 'var(--text-dim)' }}>
                      {entry.cals} kcal • P: {entry.pro}g • C: {entry.carbs}g • F: {entry.fat}g
                    </div>
                  </div>
                  <button
                    onClick={() => handleDeleteEntry(idx)}
                    style={{
                      background: 'none', border: 'none', color: 'var(--accent-red, #ff6b6b)',
                      cursor: 'pointer', padding: '4px 8px', fontSize: '1.2rem', fontWeight: 'bold'
                    }}
                    title="Remove entry"
                  >×</button>
                </div>
